...
function generateSkuList() {
  ...
  skuList.forEach((item: any) => {
    ...
    const skus = goodsInfo.value.skuList.filter(
      (sku: any) =>
        sku.specIdArr.length === specIdArr.length && sku.specIdArr.every((a: any) => specIdArr.some((b: any) => a === b)) && specIdArr.every((x: any) => sku.specIdArr.some((y: any) => x === y)),
    ) // 数据库的SKU列表
  ...
  specFormRef.value.validate((specValid: any) => {
    if (specValid) {
      skuFormRef.value.validate((skuValid: any) => {
        if (skuValid) {
          // 重组商品的规格和SKU列表
          const submitsData = Object.assign({}, goodsInfo.value)
          ...
          const skuList = JSON.parse(JSON.stringify(state.skuForm.skuList))
